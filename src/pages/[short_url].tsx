...
const appLinkMappings = [
  ...
  },
  {
    ...
    appScheme: (match: string[]) => {
      if (match[2].startsWith("in/")) {
        return `linkedin://in/${match[2].split("/")[1]}`;
      }
      return `https://www.linkedin.com/${match[2]}`;
    },
    webFallback: (match: string[]) => `https://www.linkedin.com/${match[2]}`,
  },
  {
  ...
  },
  {
    ...
    appScheme: (match: string[]) => {
      if (match[3]) {
        return `snssdk1128://aweme/detail/${match[3]}`;
      }
      if (match[4]) {
        return `snssdk1128://user/profile/${match[4].replace('@', '')}`;
      }
  ...
  },
  {
    ...
    appScheme: (match: string[]) => {
      if (match[3]) {
        return `snapchat://add/${match[3]}`;
      }
      if (match[4]) {
        return `snapchat://story/${match[4]}`;
      }
      return `snapchat://`;
    },
    webFallback: (match: string[]) => `https://www.snapchat.com/${match[2]}`,
  },
  {
    name: "Telegram",
    urlPattern: /https:\/\/(www\.)?t\.me\/([^/?#&]+)/,
    appScheme: (match: string[]) => `tg://resolve?domain=${match[2]}`,
    webFallback: (match: string[]) => `https://t.me/${match[2]}`,
  },
  {
    name: 'Spotify',
    urlPattern:
      /https:\/\/(open\.)?spotify\.com\/(track\/([^/?#&]+)|album\/([^/?#&]+)|playlist\/([^/?#&]+)|user\/([^/?#&]+))/,
    appScheme: (match: string[]) => {
      if (match[3]) return `spotify://track/${match[3]}`;
      if (match[4]) return `spotify://album/${match[4]}`;
      if (match[5]) return `spotify://playlist/${match[5]}`;
      if (match[6]) return `spotify://user/${match[6]}`;
      return null;
    },
    webFallback: (match: string[]) => `https://open.spotify.com/${match[2]}`,
  },
];
