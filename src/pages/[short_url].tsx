...
function getAppLink(url: string) {
  for (let app of appLinkMappings) {
    const match = url.match(app.urlPattern);
    if (match) {
      const appDeepLink = `${app.appScheme}${match[0]}`;
      return {
        appDeepLink,
        fallbackLink: app.webFallback + match[0],
      };
    }
  }
  return { fallbackLink: url };
...
function openLink(url: string) {
  const { appDeepLink, fallbackLink } = getAppLink(url);
  const hasGrantedPermission = localStorage.getItem('hasGrantedPermission');

  if (isMobile() && appDeepLink) {
    if (!hasGrantedPermission) {
      // Ask for permission only once
      if (confirm('Open in app?')) {
        localStorage.setItem('hasGrantedPermission', 'true');
      } else {
        window.location.href = fallbackLink;
        return;
      }
